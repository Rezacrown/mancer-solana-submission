Project Brief: Solana Crowdfunding Platform
What We're Building
...
Log: "Campaign created: goal={goal}, deadline={deadline}" 2. Contribute
...
Log: "Contributed: {amount} lamports, total={raised}" 3. Withdraw
...
Log: "Withdrawn: {amount} lamports" 4. Refund
...
Testing Checklist
Create a campaign with goal=1000 SOL, deadline=tomorrow
Contribute 600 SOL → should succeed, raised=600
Contribute 500 SOL → should succeed, raised=1100
Try withdraw before deadline → should fail
Wait until after deadline → withdraw should succeed
Try withdraw again → should fail (already claimed)
Common Pitfalls
❌ Don't send donations directly to creator ✅ Do use PDA vault

❌ Don't allow withdrawal before deadline ✅ Do check both goal AND time

❌ Don't forget to mark claimed=true ✅ Do prevent double withdrawals

❌ Don't use unwrap() everywhere ✅ Do handle errors properly

Resources
Solana Cookbook - PDAs
Solana Cookbook - CPIs
Deliverables
Rust program code
